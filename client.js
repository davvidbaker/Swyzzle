...
ipc.on('screen', function (event, img) {
  console.log('event', event)
  console.log('img', img)
  windowImage = img.image
...
var canvas = document.getElementById("myCanvas");
    ...
    var multiX = windowImage.width / canvas.width;
    var multiY = windowImage.height / canvas.height;
    // Get color at 2, 3.
    const imgData = windowImage.image;

    let ind = 0;
    for (let i = 0; i < 10000000; i+=4) {
      ctx.fillStyle = `rgb(${imgData[i*multiX]},${imgData[i*multiX+1]},${imgData[i*multiX+2]})`//);//*multiX,i*multiY),16);
